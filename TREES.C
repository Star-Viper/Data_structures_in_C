// Trees-insertion,preorder,inorder,postorder,search,delete node,count,height,mirror
#include <stdio.h>
#include <conio.h>
#include <stdlib.h>
...
struct Node *deletee(struct Node *r, int data)
{
	struct Node *temp;
	if (r == NULL)
	{
		...
		r->left = deletee(r->left, data);
	else if (data > r->value)
		r->right = deletee(r->right, data);
	else if (r->left != NULL && r->right != NULL)
	{
		temp = findMin(r->right);
		r->value = temp->value;
		r->right = deletee(r->right, r->value);
	}
	else
	{
		temp = r;
...
switch (ch) {
		...
		case 6:
			...
			break;
		case 7:
			printf("Count of the nodes is: %d", count(root));
			break;
		case 8:
